...
The software-validation evidence in this chapter begins with the full application audit performed on 31 July 2026 and includes the foundation-model work completed on 14 August 2026. Historical reports are retained for chronology because test counts, routes, and capabilities changed over time. The audit uses backend tests, frontend quality commands, browser tests, artifact checks, real-wrapper smoke tests, and source review. The previous coordinated Azure validation still targets commit \texttt{6796b1e}; the new repository release is locally verified but not represented as already promoted to Azure.
...
ESLint passed, TypeScript type checking passed, and the production build completed with 25 routes. The current Playwright matrix executes 198 tests across desktop Chromium, Firefox, WebKit, and three responsive projects. It covers 360-pixel, 390-pixel iPhone, and 768-pixel profiles. The new monthly case verifies that the capability remains daily, reports its long-history readiness gate, and never offers 30 days of demonstration data as a false bypass. Populated 167-hour and 169-hour local-week tests continue to exercise daylight-saving boundaries.
...
The previously validated six-service Compose stack ran with PostgreSQL, backend, and frontend healthy and all workers running. For the new release, Compose configuration resolves with the month feature flag and the production Dockerfile stages the exact adapter, pinned dependencies, and base-model prefetch step. The full foundation image remains subject to the deployment memory/startup gate documented for the target environment.
