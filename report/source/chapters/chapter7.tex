...
The software-validation evidence in this chapter begins with the full application audit performed on 31 July 2026 and ends with the jury-release gate on 9 August 2026. Historical reports are retained for chronology because test counts, routes, and capabilities changed over time. The audit used both SQLite and a disposable PostgreSQL database, frontend quality commands, a six-project browser matrix, dependency audits, artifact checks, Docker health, and source review. The checkpoint quantile reanalysis and the separate serving-normalization evaluation were generated on 1 August 2026; each is identified by script, input, checkpoint, environment, and output hashes. The final Azure validation targeted commit \texttt{af467989694322ef76a5402a367893ea01f5e1dd}; it did not rerun or alter any scientific experiment.

\section{Backend validation}

The final local backend suite passed 106 tests with four expected PostgreSQL-only skips under SQLite. In the earlier full audit, a uniquely named disposable PostgreSQL database was created on the running server, migrated from an empty schema through all 17 Alembic revisions, tested, and removed in cleanup. That production-equivalent result remains 100 passed and zero skipped in 42.78 seconds, with one warning. No schema migration changed between the validated baseline and the jury release.
...
ESLint passed, TypeScript type checking passed, and the production build completed with 25 routes. The final Playwright matrix executed across desktop Chromium, Firefox, WebKit, and three responsive projects and passed 180 of 180 tests. It covers 360-pixel, 390-pixel iPhone, and 768-pixel responsive profiles in addition to desktop browsers. Populated 167-hour and 169-hour local-week tests also exercise daylight-saving boundaries rather than only empty-period behavior.
...
\midrule
Backend on SQLite & 106 passed, 4 skipped & Final local suite; PostgreSQL-only behavior excluded \\
Backend on isolated PostgreSQL & 100 passed, 0 skipped & Historical full-audit evidence; schema unchanged \\
...
Playwright six-project matrix & 180 passed, 0 failed & Desktop and three responsive profiles green \\
...
Azure public deployment & HTTPS web/API, email worker, managed PostgreSQL, both models warmed & Email and Google capability verified; alert/avatar workers remain absent \\
\bottomrule
\end{tabularx}
\end{table}
