...
Database/API & Current local pytest suite & 112 passed, 4 PostgreSQL-only skips \\
...
Browser E2E & Playwright six-project matrix & 192 passed, 0 failed \\
...
Deployment & Docker health plus Azure HTTPS/browser smoke & Public web/API, email worker, managed database, both models, SMTP capability, and Google origin verified \\
\bottomrule
\end{longtable}
...
The verified Azure deployment supplies managed TLS, Azure hostnames, private images, externalized secrets, a healthy email worker, ready SMTP capability, and a production Google identity origin. The current web revision is \texttt{v1-f807316}; API and email worker use \texttt{v1-6796b1e}. Remaining production work includes an application security-header policy, custom domain, managed identity, durable avatar storage, centralized monitoring, Azure alert/avatar workers, and a tested backup-restore procedure. Older inactive revisions remain available, but rollback selection must preserve compatible web and API versions.
