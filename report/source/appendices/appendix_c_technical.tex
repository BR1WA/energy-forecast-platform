...
Database/API & Current local pytest suite & 116 passed, 4 PostgreSQL-only skips \\
Database/API & Current CI PostgreSQL and test-image gates & Passed \\
Frontend static & ESLint and TypeScript & Both passed \\
Frontend build & Next.js production compilation & Passed, 25 routes \\
Browser E2E & Playwright six-project matrix & 198 passed, 0 failed \\
...
Dependency security & npm and production pip audits & Zero known vulnerabilities \\
Deployment & Docker health plus Azure HTTPS/browser smoke & Prior public web/API, email worker, managed database, both TFTs, SMTP capability, and Google origin verified \\
\bottomrule
\end{longtable}
...
The verified Azure deployment supplies managed TLS, Azure hostnames, private images, externalized secrets, a healthy email worker, ready SMTP capability, and a production Google identity origin. The current web revision is \texttt{v1-f807316}; API and email worker use \texttt{v1-6796b1e}. This verified revision predates the repository's 30-day model integration. A later cloud promotion must use the production backend image that caches the pinned foundation base, enable \texttt{FORECAST\_30D\_ENABLED}, and pass model warm-up and memory checks. Remaining production work includes an application security-header policy, custom domain, managed identity, durable avatar storage, centralized monitoring, Azure alert/avatar workers, and a tested backup-restore procedure.
