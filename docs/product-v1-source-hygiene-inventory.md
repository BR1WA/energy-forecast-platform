...
**Audit phase:** read-only inventory, dependency verification, and corrective classification (Phase 1-3)
...
This document records an audit only. No tracked or untracked artifact was
deleted, moved, renamed, staged, committed, or rewritten. No history-rewrite
or cleanup command was used. No external preservation destination has been
invented or assumed; therefore the preservation gate is not satisfied and no
removal may begin.
...
- 2 additional non-extension research outputs: `models/archive/config.json`
  and `results/benchmark.md`.
...
- Final classification totals after the corrective review are: 4
  `RUNTIME_REQUIRED`, 50 `RESEARCH_VALUABLE`, 2 `REGENERABLE`, and 3
...
| `results/benchmark.csv` | CSV generated benchmark | 1,943 | No runtime reference; generated by `scripts/collect_benchmarks.py` | `REGENERABLE` | Regenerate from experiment metrics, then remove after documentation | Benchmark snapshot is lost if not preserved as research evidence. |
...
| `results/benchmark.md` | Markdown generated report | 2,591 | No runtime reference; generated by `scripts/collect_benchmarks.py` | `REGENERABLE` | Regenerate from experiment metrics, then remove after documentation | Loss of a human-readable benchmark snapshot if not preserved. |
...
- `results/benchmark.csv` and `results/benchmark.md` are generated by
  `scripts/collect_benchmarks.py` from experiment metrics.
...
and cleanup implementation have not started.

## Preservation gate and stop condition

No approved external storage location or separate research repository/commit
was provided for the `RESEARCH_VALUABLE` rows. They remain untouched. The next
authorized action is to provide written preservation evidence, then perform the
Phase 4 preservation gate. Cleanup implementation must not begin until that
gate is satisfied.
...
- `results/benchmark.csv` and `results/benchmark.md` are the 2
  `REGENERABLE` outputs with an identified collector and currently available
  local metrics inputs. Removal still requires documenting those inputs in the
  eventual cleanup change; this phase did not execute the writer.
...
The corrective review changed three classifications from `REGENERABLE` to
`UNKNOWN_REQUIRES_REVIEW` because their origin, inputs, or exact generator could
not be established. They remain untouched and cannot be removed.
...
| `results/benchmark.csv` | `python scripts/collect_benchmarks.py` | 18 local `experiments/**/metrics.json` files exist; 15 match the collector's expected dataset/lookback/horizon/model/timestamp layout, and optional metric fields default safely | `results/benchmark.csv` | Remains `REGENERABLE`; generator and sufficient current local inputs verified by source inspection. Not executed because it writes output. |
| `results/benchmark.md` | `python scripts/collect_benchmarks.py` | Same 15 collector-compatible experiment metric groups as the CSV | `results/benchmark.md` | Remains `REGENERABLE`; generator and sufficient current local inputs verified by source inspection. Not executed. |
...
The preservation manifest is complete for the currently classified 50
`RESEARCH_VALUABLE` rows. It is planning metadata only; no external location
has been supplied, so the preservation gate remains unsatisfied.

## Phase 3 integrity result and stop condition
...
files in this checkout only; they do not prove external preservation. The three
unknown artifacts and all 50 research artifacts remain in place because no
approved preservation destination exists. Phase 4 preservation and all cleanup
