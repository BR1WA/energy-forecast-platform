...
def test_forecast_api_persists_an_owned_explicit_fallback(monkeypatch):
    ...
    try:
        ...
        day_timestamps = [datetime.fromisoformat(point["timestamp"]) for point in payload["points"]]
        assert all(right - left == timedelta(hours=1) for left, right in zip(day_timestamps, day_timestamps[1:]))
        ...
        week_timestamps = [datetime.fromisoformat(point["timestamp"]) for point in week_payload["points"]]
        assert all(right - left == timedelta(hours=1) for left, right in zip(week_timestamps, week_timestamps[1:]))
...
@pytest.mark.skipif(importlib.util.find_spec("torch") is None, reason="PyTorch is required for the real dual-horizon API test")
def test_real_dual_horizon_api_uses_packaged_models_and_preserves_ownership(monkeypatch):
    engine = create_engine(
        "sqlite:///:memory:",
        connect_args={"check_same_thread": False},
        poolclass=StaticPool,
    ...
    db = TestingSession()
    try:
        owner = User(email="real-forecast-owner@example.com", password_hash=hash_password("password123"), role="user", is_active=True)
        other = User(email="real-forecast-other@example.com", password_hash=hash_password("password123"), role="user", is_active=True)
        db.add_all([owner, other])
        db.commit()
        site = ensure_default_site(db, owner.id)
        site.timezone = "UTC"
        ...
        assert [row["model"]["name"] for row in capability_rows] == ["global_tft_24h", "global_tft_168h"]
        assert all(row["model"]["available"] and row["model"]["warmed"] for row in capability_rows)

        generated = {}
        for horizon, model_name, fingerprint in (
            (24, "global_tft_24h", "60fedcdee375dc0b2973e55b9f4ec752da69c2a7e390e1f951ed5cbb7bc5a04d"),
            (168, "global_tft_168h", "80af16b25af9b912019c6e40cfdc491e2cf5173e5743144596801e28df695f93"),
        ):
            readiness = client.get(f"/api/v1/forecast/readiness?horizon_hours={horizon}", headers=owner_headers)
            assert readiness.status_code == 200
            assert readiness.json()["status"] == "ready"
            assert readiness.json()["model"]["name"] == model_name

            response = client.post("/api/v1/forecast/run", headers=owner_headers, json={"horizon_hours": horizon})
            ...
            timestamps = [datetime.fromisoformat(point["timestamp"]) for point in payload["points"]]
            assert all(right - left == timedelta(hours=1) for left, right in zip(timestamps, timestamps[1:]))
            assert datetime.fromisoformat(payload["forecast_end"]) - datetime.fromisoformat(payload["forecast_start"]) == timedelta(hours=horizon)

            latest = client.get(f"/api/v1/forecast/latest?horizon_hours={horizon}", headers=owner_headers)
            assert latest.status_code == 200
            assert latest.json()["id"] == payload["id"]
            assert client.get(f"/api/v1/forecast/latest?horizon_hours={horizon}", headers=other_headers).json() is None
